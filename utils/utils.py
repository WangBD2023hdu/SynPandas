...
def split_table_for_multipage(html_table, max_rows_per_page=None):
    """
    将HTML表格分割成多个部分，用于跨页显示。
    返回一个列表，每个元素是一个表格HTML片段。
    
    Args:
        html_table: HTML表格字符串
        max_rows_per_page: 每页最大行数（不包括表头），如果为None则自动计算
    ...
    # 如果没有指定每页行数，使用默认值（大约15-20行，取决于内容）
    ...
    for start_idx in range(0, total_rows, max_rows_per_page):
        ...
        new_table.attrs = table.attrs.copy()
        ...
        for row in body_rows[start_idx:end_idx]:
