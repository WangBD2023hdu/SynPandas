...
class chrome_render:
    ...
    def detect_cross_page_table(self):
        """
        检测页面中是否有跨页表格
        返回跨页表格的信息列表
        """
        table_info = self.driver.execute_script('''
            ...
            const tables = document.querySelectorAll('.table_outer table');
            ...
            tables.forEach((table, index) => {
                ...
                if (tableBottom > pageHeight) {
                    // 获取表格的HTML内容
                    const tableOuter = table.closest('.table_outer');
                    let caption = '';
                    let footnote = '';
                    
                    if (tableOuter) {
                        const captionEl = tableOuter.querySelector('.table_caption');
                        const footnoteEl = tableOuter.querySelector('.table_footnote');
                        if (captionEl) caption = captionEl.textContent.trim();
                        if (footnoteEl) footnote = footnoteEl.textContent.trim();
                    }
    ...
    def generate_multipage_pdf(self, file_path, save_path, enable_multipage=True):
        """
        生成多页PDF，支持跨页表格
        
        Args:
            file_path: HTML文件路径
            save_path: 保存PDF的路径（不含扩展名）
            enable_multipage: 是否启用多页支持
            
        Returns:
            cross_column_paragraphs: 页面元素信息
        """
        try:
        ...
        cross_page_tables = []
        if enable_multipage:
            cross_page_tables = self.detect_cross_page_table()
        
        # 如果没有跨页表格，使用原来的单页逻辑
        if not cross_page_tables or not enable_multipage:
            return self.get_location(file_path, save_path)
        ...
        # 生成多页PDF
        # 注意：Chrome的printToPDF默认支持多页，如果内容超出会自动分页
        # 但我们需要确保表格能够正确跨页显示
